...
public:
    ...
    //функция сложения двух векторов
     ...
     std::vector <std::vector<std::vector<std::vector<double>>>> initialize_weights(int input_size, int hidden_neurons, int output_size, int  hidden_layers);

    
     std::vector < std::vector<std::vector<double>>> initialize_biases(int input_size, int hidden_neurons, int output_size, int  hidden_layers);
    ...
    double Train(const std::vector<double>& x, const std::vector<double>& y, int epochs, double learning_rate);
